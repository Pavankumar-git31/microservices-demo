...
sed -i "s|^DOCKER_VERSION=.*|DOCKER_VERSION=\"$DOCKER_VERSION\"|" Contanierize/dockerbuild.sh
# update dockerpush.sh
sed -i "s|^DOCKER_USER=.*|DOCKER_USER=\"$DOCKER_USER\"|" Contanierize/dockerpush.sh
sed -i "s|^DOCKER_VERSION=.*|DOCKER_VERSION=\"$DOCKER_VERSION\"|" Contanierize/dockerpush.sh
